...
import { setAllProductsData, setAllVendorsData } from '@/app/redux/slices/vendors/vendordata'
...
const ProductApproved = () => {
  ...
  const pendingProducts: IProduct[] = Array.isArray(allProductsData)
    ? allProductsData.filter((p: IUser) => p.verificationStatus === 'pending')
    : []

  const approvedCount = Array.isArray(pendingProducts)
    ? allProductsData.filter((p: IUser) => p.verificationStatus === 'approved').length
    : 0

  const rejectedCount = Array.isArray(allProductsData)
    ? allProductsData.filter((p: IUser) => p.verificationStatus === 'reject').length
    : 0
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] text-white px-4 py-8 md:px-8">
      ...
      >
        ...
        >
          {[
            { label: 'Pending', value: pendingProducts.length, color: '#facc15' },
        ...
        <AnimatePresence mode="popLayout">
          {pendingProducts.length === 0 ? (
            ...
            >
              <AnimatePresence mode="popLayout">
                {pendingProducts.map((product: IProduct, i: number) => {
                  const vid = String(product._id)
                  const isActive = activeProductId === vid
